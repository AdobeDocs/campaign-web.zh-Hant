...
傳送的訊息數反映了您的傳遞率。 永遠不能是100%，原因如下：部分地址或電話號碼可能錯誤，電子郵件提供者的垃圾郵件封鎖者可能會拒絕您的郵件，或可能發生傳遞問題。

此 **已傳遞** 指標針對每個管道顯示下列KPI：

* 成功傳遞的訊息數相對於已傳送訊息總數的百分比。
...
透過追蹤訊息開啟的總次數來計算開啟總計，無論開啟次數是由多少位收件者產生。 此指標僅適用於電子郵件。
...
您可以在訊息內容中新增URL，將收件者重新導向至特定頁面。 點進率會測量收件者點按訊息中連結的數量和百分比。
...
收件者必須能夠透過電子郵件內容中的專用取消訂閱連結，或透過在簡訊中回覆STOP，選擇退出電子郵件和簡訊。
...
您的Adobe Campaign平台所傳送的某些訊息可能不會到達目的地。 當使用者地址或電話有拼寫錯誤、收件者變更其電子郵件地址或其信箱已滿時，就可能發生這種情況。 如果訊息無法傳送至設定檔，遠端伺服器會自動傳送錯誤訊息至Adobe Campaign。 此錯誤適用於判斷應隔離電子郵件地址、電話號碼或裝置。

因此，您應該一律檢查並更新資料庫，並確定所有設定檔都是有效且真實的。 傳送錯誤可能是暫時或永久性的（軟退信或硬退信），具體取決於未傳送訊息的原因。
