...
型別可讓您標準化所有傳遞的業務實務。 **型別**&#x200B;是&#x200B;**型別規則**&#x200B;的集合，可讓您控制、篩選傳遞內容，並排定其優先順序。 在準備階段，與類型規則中的標準相符的輪廓會排除在傳遞客群之外。
...
>title="類型屬性"
>abstract="定義型別的屬性，並展開&#x200B;**[!UICONTROL 其他選項]**&#x200B;區段以存取進階設定。 使用&#x200B;**[!UICONTROL IP相關性]**&#x200B;欄位將IP相關性與型別建立關聯。 這可讓您定義哪些特定IP位址可用於每個相似性，藉此更好地控制傳出SMTP流量。"
...
## 建立型別規則 {#typology-rule}

>[!CONTEXTUALHELP]
>id="acw_business_rules_typology_rules_properties"
>title="類型規則屬性"
>abstract="定義型別規則的屬性。 **控制**&#x200B;規則會驗證訊息品質和預先傳送的有效性，而&#x200B;**篩選**&#x200B;規則會根據特定條件排除目標對象的區段。<br/><br/>您也可以變更規則的執行順序，以管理在同一訊息處理階段執行相同型別的數個規則時，型別規則的執行順序。"
...
1. 選取型別規則的&#x200B;**[!UICONTROL 型別]**：
   ...
   * **篩選**&#x200B;規則會根據特定條件（例如年齡、地點、國家/地區、電話號碼）排除目標對象的區段。 這些規則會連結至目標維度。
...
>title="篩選"
>abstract="**篩選**&#x200B;規則會根據特定條件（例如年齡、地點、國家/地區、電話號碼）排除目標對象的區段。 選取型別規則的目標維度，然後按一下&#x200B;**[!UICONTROL 新增規則]**&#x200B;按鈕以存取查詢建模器並建置規則。"

>[!CONTEXTUALHELP]
>id="acw_business_rules_typology_rules_code"
>title="程式碼"
>abstract="**控制**&#x200B;規則會驗證訊息品質和預先傳送的有效性（例如，字元顯示、簡訊長度、位址格式、URL縮短）。 它們是使用JavaScript程式碼建立的。"
