...
>title="資源有效期限制"
>abstract="「有效性限制」欄位是用於上傳的資源，例如鏡像頁面或影像。 這些資源在限定時間內有效：一旦達到限制，資源將不再可用。"
...
**資源有效性限制** 用於上傳的資源，主要用於鏡像頁面和影像。 本頁上的資源在限定時間內有效（以節省磁碟空間）。在此限制後，這些資源將不再可用。
...
* **[!UICONTROL 強制產生鏡像頁面]**:即使傳送中未插入鏡像頁面的連結，仍使用此模式來產生鏡像頁面。
* **[!UICONTROL 不生成鏡像頁]**:使用此模式可避免產生鏡像頁面，即使傳送中有連結亦然。
* **[!UICONTROL 生成僅使用消息標識符可訪問的鏡像頁]**:當電子郵件內容中不存在鏡像頁面連結時，請使用此選項從客戶端控制台啟用在傳遞日誌窗口中訪問鏡像頁面的內容。
