...
>title="客群"
>abstract="從此畫面中，您可以存取可作為工作流程和獨立傳遞目標之所有對象的清單。按一下「**建立**」，在視覺畫布中建立新對象。<br/><br/>除了從頭開始建立簡單的對象之外，您還可以利用工作流程活動來調整對象。例如，您可以將多個對象合併為單一對象、使用外部屬性豐富對象，或根據您選擇的規則將其劃分為多個對象。"
...
Campaign Web可讓您在視覺化工作流程畫布中建立新對象。 除了從頭開始建立簡單的客群之外，您還可以利用工作流程活動來調整客群。例如，您可以將多個對象合併為單一對象、使用外部屬性豐富對象，或根據您選擇的規則將其劃分為多個對象。
