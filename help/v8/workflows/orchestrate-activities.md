...
若要移除活動，請在畫布中選取該活動，然後按一下活動屬性中的&#x200B;**刪除**&#x200B;圖示。 如需可用的選項，請參閱[刪除並中斷連線活動](#delete)。
...
* 從畫布中&#x200B;**刪除**&#x200B;活動。 請參閱[本節](#delete-activity)。
...
若要刪除活動，請在畫布中選取該活動，然後按一下活動屬性中的&#x200B;**刪除**&#x200B;圖示。 確認對話方塊隨即顯示。

* 如果活動未連線至任何其他活動，請確認將其刪除。

  ![刪除活動簡單](assets/workflow-delete.png)

* 如果活動已連線至一或多個後續活動，請選擇處理方式：

  ![刪除活動多個](assets/workflow-delete2.png)

  * **刪除所有後續活動**：移除相同路徑上的活動及其後的所有活動。
  * **僅刪除此活動**：僅移除選取的活動，並重新連線剩餘的路徑。 此選項僅在活動具有單一後續活動時可用。
  * **刪除並建立新分支**：移除選取的活動但保留其後續活動，將其移至新的獨立分支。

按一下&#x200B;**刪除**&#x200B;以確認您的選擇，或按一下&#x200B;**取消**&#x200B;以關閉對話方塊而不刪除任何內容。

### 中斷轉換連線 {#disconnect-transition}
...
1. 按一下轉變屬性中的&#x200B;**中斷連線**&#x200B;圖示。
   ...
   轉換屬性窗格中的![中斷連線圖示](assets/workflow-transition.png)

   此圖示僅適用於轉變導向到下游活動時。 確認對話方塊隨即顯示。

1. 按一下&#x200B;**中斷連線**&#x200B;以確認，或按一下&#x200B;**取消**&#x200B;以關閉對話方塊而不中斷任何連線。

   ![中斷連線轉換確認對話方塊](assets/workflow-transition2.png)

如果來源活動有多個外站轉變（例如，具有數個結果分支的&#x200B;**分割**&#x200B;活動，或&#x200B;**分支**&#x200B;活動），您可以從活動自己的屬性窗格中個別移除其中的任何一個：

1. 選取活動，然後在&#x200B;**區段**&#x200B;區段中找出您要移除的轉變。

1. 按一下該轉變旁的垃圾桶圖示。 確認對話方塊隨即顯示。

   區段結果旁的![垃圾桶圖示](assets/workflow-transition3.png)

1. 按一下&#x200B;**移除**&#x200B;以確認，或按一下&#x200B;**取消**&#x200B;以關閉對話方塊而不移除任何內容。

   ![移除轉換確認對話方塊](assets/workflow-transition4.png)
...
>title="活動屬性"
>abstract="您可以透過此區段設定活動的執行設定，包括要執行的動作、最長持續時間、時區、機器親和原則和非同步行為。"
