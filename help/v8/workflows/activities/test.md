...
>title="條件"
>abstract="「**測試**」活動可以有多個輸出轉換。在工作流程執行期間，將相繼測試每個條件，直到滿足其中一個條件。如果沒有滿足任何條件，工作流程將沿著「**[!UICONTROL 預設條件]**」的路徑繼續。如果未啟動預設條件，工作流程會在此點停止。"
...
1. 在工作流程執行期間，將相繼測試每個條件，直到滿足其中一個條件。如果沒有滿足任何條件，工作流程將沿著「**[!UICONTROL 預設條件]**」的路徑繼續。如果未啟動預設條件，工作流程會在此點停止。
