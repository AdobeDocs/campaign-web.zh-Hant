...
>[!NOTE]
>
>**建置對象**&#x200B;活動（查詢型別）也支援&#x200B;**擴充資料**。 請參閱[深入瞭解](build-audience.md#build-audience-configuration)。
...
* **外部資料庫**：從儲存在外部資料庫的資料表新增欄位。 [了解更多資訊](#external-data)。
...
**擴充資料**&#x200B;區段可在&#x200B;**擴充**&#x200B;和&#x200B;**建置對象** （查詢型別）活動中使用。 它可讓您使用資料庫中的其他資訊（例如合約參考或電子報訂閱）來增強目標資料。 該資料會與工作流程&#x200B;**工作表格**&#x200B;中的對象一起儲存，並可供後續的活動使用。 您可以新增單一擴充屬性、集合連結或運算式，並存取進階選項。

按一下&#x200B;**新增擴充資料**，並選取要用於擴充的屬性。 [瞭解如何選取屬性並將其新增至我的最愛](../../get-started/attributes.md)。
...
新增至少一個擴充屬性後，按一下&#x200B;**[!UICONTROL 進階引數]**&#x200B;以設定擴充資料的建置方式，包括分組、重複資料刪除、主要金鑰處理和傳入事件資料。 這些選項反映了使用者端主控台，並適用於進階工作流程案例。

![顯示擴充進階引數的熒幕擷圖](../assets/workflow-query-advanced-parameters.png)

>[!NOTE]
>
>**建置對象**&#x200B;與&#x200B;**擴充**&#x200B;活動之間的可用選項不同。

每個活動皆可使用下列選項：

+++ 建立對象活動（查詢型別）

* **[!UICONTROL 保留主集的所有額外資料]**：保留輸出轉變中主輸入集的額外資料行。
* **[!UICONTROL 依目標維度元素將資料分組]**：將結果分組，以便每個目標籤錄僅顯示一次。
* **[!UICONTROL 移除重複資料列(DISTINCT)]**：從結果集中移除重複資料列。
* **[!UICONTROL 停用目標維度主索引鍵的自動新增]**：防止活動自動將目標維度主索引鍵新增至結果。
* **[!UICONTROL 停用0識別碼記錄的自動篩選]**：保留識別碼值為0的記錄，而非自動篩選它們。
* **[!UICONTROL 使用入站事件資料]**：使用來自入站轉變的資料做為活動的工作輸入。
...
* **[!UICONTROL 移除重複資料列(DISTINCT)]**：從結果集中移除重複資料列。
* **[!UICONTROL 停用0識別碼記錄的自動篩選]**：保留識別碼值為0的記錄，而非自動篩選它們。
* **[!UICONTROL 為結果的每一行新增識別碼]**：為每個輸出列新增唯一識別碼。
...
## 利用外部資料庫資料擴充 {#external-data}

**擴充**&#x200B;和&#x200B;**建置對象** （查詢型別）活動可讓您使用&#x200B;**[!UICONTROL 同盟資料存取(FDA)]**&#x200B;外部帳戶，從儲存在外部資料庫的表格新增欄位。 [瞭解如何設定外部帳戶](../../administration/create-external-account.md)。

>[!NOTE]
>
>只有簡單連線可用於調解外部資料庫欄位。 此型別的擴充不支援進階聯結條件。

若要新增外部資料庫欄位，請執行下列步驟：

1. 在活動窗格的&#x200B;**[!UICONTROL 擴充資料]**&#x200B;區段中，按一下&#x200B;**[!UICONTROL 新增外部資料庫欄位]**。
1. 選取要使用的FDA外部帳戶，然後瀏覽或輸入要使用的表格名稱。
1. 選取您需要的資料行，然後按一下[新增]。**&#x200B;**

   ![顯示外部資料庫按鈕的熒幕擷圖](../assets/workflow-enrichment8.png)

1. 在&#x200B;**[!UICONTROL 調解條件]**&#x200B;區段中，定義&#x200B;**[!UICONTROL Source]**&#x200B;屬性和&#x200B;**[!UICONTROL Destination]**&#x200B;外部資料行以調解資料。 您可以按一下&#x200B;**[!UICONTROL 新增聯結]**，以新增數個聯結。

1. 在&#x200B;**[!UICONTROL 收集的資料]**&#x200B;下拉式清單中，選擇收集相符資料列的方式：

   * **[!UICONTROL 收集所有列]**：擷取每個相符的列。
   * **[!UICONTROL 限制列數]**：最多擷取您定義的列數。 啟動&#x200B;**[!UICONTROL 啟用排序]**&#x200B;以定義用來決定要保留哪些列的順序。
   * **[!UICONTROL 單列（專家）]**：假設單列符合來最佳化查詢。 如果此假設不正確，結果可能有誤（資料遺失或重複）。
   * **[!UICONTROL 彙總列]**：當收集的欄已包含彙總值（例如計數或平均值）時，請使用此選項。
   * **[!UICONTROL 合併]**：將相符的資料列合併為單一結果。

   >[!NOTE]
   >
   >針對&#x200B;**[!UICONTROL 限制列數]**&#x200B;和&#x200B;**[!UICONTROL 合併]**&#x200B;選項，您可以啟用&#x200B;**[!UICONTROL 啟用排序]**&#x200B;以定義用來決定保留哪些列的順序或定義合併順序。

1. 或者，按一下&#x200B;**[!UICONTROL 建立篩選器]**&#x200B;以使用查詢模型工具來篩選外部資料。 [瞭解如何使用查詢模型工具](../../query/query-modeler-overview.md)。

   ![顯示外部資料庫組態的熒幕擷圖](../assets/workflow-enrichment8.png)

新增的欄位會使用活動名稱和表格名稱加上標籤。
...
在這裡，我們只新增單一擴充屬性，例如出生日期。 請依照下列步驟操作：
