...
* [建立對象](build-audience.md)：定義您的目標母體。 選取現有對象，或使用查詢建模器定義您自己的查詢。 您也可以設定&#x200B;**擴充資料** （可用於查詢）。
...
* [更新資料](update-data.md)：對資料庫中的欄位執行大量更新。 您可以透過數個選項，依據個人需求設定資料更新。
...
>title="結束活動"
>abstract="「**開始**」活動可讓您以圖形方式標記工作流程的起點。 此活動並不會造成功能性影響，因此為選用。"
...
* **開始**&#x200B;和結束[結束](end.md)：以圖形方式標示工作流程的開始和結束。 它們是選擇性的，本身沒有功能影響。

  >[!NOTE]
  >
  >請注意，**開始**&#x200B;活動會自動新增至26.3版之前建立的所有工作流程。 依預設，不會將其插入新的工作流程中。
