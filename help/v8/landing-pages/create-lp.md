...
>title="定義預先載入選項"
>abstract="當選取&#x200B;**「預先填寫表單中所參考的資料」**&#x200B;選項時，如果登陸頁面的訪客與資料庫中的設定檔相符，設定檔的資訊會自動預先載入表單中。選取「**授權缺少 ID**」選項後，任何訪客 (包括匿名使用者) 都可以存取登入頁面。"
...
1. 登入頁面可以有後續頁面。 若要新增頁面，請瀏覽&#x200B;**[!UICONTROL 頁面]**&#x200B;區段，然後按一下您想要為此登入頁面設計的每個頁面的&#x200B;**[!UICONTROL 編輯內容]**&#x200B;按鈕。 每個頁面的內容均已預先填滿。 視需要編輯。 [了解更多資訊](lp-content.md)。
