---
title: Campaign v8 Web 使用者介面發行說明
description: 探索最新 Campaign Web 使用者介面版本的新功能
exl-id: a0d2ab24-1854-4ad6-8a8c-b55488b20bf9
TQID: https://experienceleague.adobe.com/HkI2JUqLNM805hPfVsXl-8nwR70TzxRP31V9EI4yKGA
product_v2: id: dfc56824-e8b9-499e-85d4-21aedb507314
feature_v2: id: a075b2c1-7748-4328-b7f6-343aa314616aid: c309ee4e-82e4-4f7e-b608-ef345678c34eid: d5ef99fa-df0c-4153-bf94-105ad0724167
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550ccid: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: d6b9b9c9905d840e65ac0aa267a665997c6e6b16
workflow-type: tm+mt
source-wordcount: 330
ht-degree: 33%

---

# 發行說明 {#latest-release}

>[!CONTEXTUALHELP]
>id="acw_homepage_learning_card2"
>title="發行說明"
>abstract="Adobe Campaign Web 使用者介面版本會在持續傳遞模型上運作，透過該模型可採用更具擴充性、分階段的方式來部署功能。 因此，Campaign 發行說明每月更新數次，提供最新功能、改進和修正。 我們建議您定期檢查。"

Adobe Campaign Web 使用者介面版本會在持續傳遞模型上運作，透過該模型可採用更具擴充性、分階段的方式來部署功能。 因此，這些發行說明每月會更新多次。 請定期進行檢查。

>[!NOTE]
>
>未來幾天將逐步新增以下功能和改善的詳細檔案。

## 2026年8月發行 {#26-8-release}

_2026年8月18日_

### 新功能 {#26-8-features}

<table>
<thead>
<tr>
<th><strong>核准工作流程活動</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p><strong>核准</strong>工作流程活動（先前僅可在使用者端主控台中使用）現在可在Campaign Web使用者介面中使用。 將工作指派給群組或個別操作員、自訂通知標題和訊息，並將可能的答案（例如「是/否」）定義為輸出分支。</p>
</td>
</tr>
</tbody>
</table>

### 功能改進 {#26-8-improvements}

* **開啟追蹤**：您現在可以直接從Campaign網頁使用者介面啟用或停用開啟追蹤。 這可協助您遵守資料保護規範。
* **方案清單檢視**：方案現在會列在專用檢視中，類似於行銷活動、傳遞和工作流程。 您可以瀏覽現有的程式，並直接從這個檢視建立新的程式。
* **自訂結構描述組態**：在&#x200B;**動作資料**&#x200B;區段中，您現在可以不允許自訂結構描述記錄上的&#x200B;**重複**動作。
  <!--* **Workflow and delivery templates (only msf???)**: When creating a new workflow or delivery, you must now explicitly select a template. A default template is no longer applied automatically.-->
* **自訂篩選器**：在結構描述編輯器中，自訂篩選器現在支援直接連結的篩選條件，而且您可以使用新的&#x200B;**自訂篩選器設定**&#x200B;覆寫任何屬性的預設標籤。
* **結構描述驗證**：您現在可以使用新的&#x200B;**驗證**&#x200B;按鈕，直接從結構描述編輯器驗證結構描述的結構。
* **資料夾安全性**：當使用者具有&#x200B;**插入**&#x200B;或&#x200B;**編輯**&#x200B;存取許可權時，**重新命名**選項現在可用於資料夾。
  <!--* **Enrichment activity**: You can now enrich data from an external database directly from the **Enrichment** workflow activity. This matches the capability already available in the Client Console.-->
