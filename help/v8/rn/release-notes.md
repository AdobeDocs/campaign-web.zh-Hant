...
* **擴充活動中的調解**：此 **擴充** 活動現在可用來調解來自Campaign資料庫結構的資料，與來自另一個結構的資料，或來自臨時結構的資料，例如使用載入檔案活動上傳的資料。 例如，您可以使用此選項將上傳檔案中指定的設定檔所在國家/地區與 Campaign 資料庫專用表中可用的國家/地區之一進行調和。[閱讀更多](../workflows/activities/enrichment.md)
