...
本指南會概述Campaign v8的主要功能，供行銷人員從Campaign Standard轉換為Campaign v8。
...
透過移轉，您從Campaign Standard匯入的所有資料都會匯入Campaign v8，以確保順暢的轉換，並將對持續作業造成的中斷降至最低。
...
如同在Campaign Standard中一樣，您可以新增控制組至您的傳送。 您可以定義控制組以避免傳送訊息給部分對象，並將傳送後的行為與主要目標進行比較。 此選項可協助您測量行銷活動的影響。
瞭解如何[設定控制組](../../v8/audience/control-group.md){target="_blank"}。

>[!AVAILABILITY]
>
>* 在轉變期間，所有透過「Campaign Standard查詢」活動建立的對象都會轉換為Campaign v8中的預先定義篩選器。 Campaign v8也支援查詢活動。
>
...
Campaign Web使用者介面已完全重新構想工作流程使用者介面，以方便使用、設定、執行和疑難排解。 由於您已在Campaign Standard方面擁有工作流程的使用經驗，您可以協調全方位的流程和任務、改善行銷活動各方面的速度和規模，從建立區段及準備訊息到傳送。 此外，您可以讓您的管道與一個易於使用的單一介面同步，用於行銷活動協調流程。
...
使用Campaign Web使用者介面，行銷人員可以從左側功能表&#x200B;**傳送**&#x200B;建立獨立傳送，或在工作流程內容中建立傳送，無論是否包含在行銷活動中。 重要步驟會與您之前在Campaign Standard中的體驗一致。 在下列章節中瞭解如何建立傳遞： [傳遞建立與管理檔案](../../v8/msg/gs-deliveries.md)。
...
>
>* 追蹤連結、映象頁面URL連結、訂閱/取消訂閱連結的運作方式與Campaign Standard相同。
>
...
1. **設計和定義內容**
   ...
   透過此影片瞭解如何上傳HTML來建立電子郵件、如何使其與電子郵件Designer相容，以及如何將其轉換為範本：
...
作為轉換至Campaign v8的Campaign Standard使用者，您現有的登入頁面已移轉至Campaign Web使用者介面。 您可以存取相同範圍的功能。
...
>[!AVAILABILITY]
>
>* [動態報告](https://experienceleague.adobe.com/en/docs/experience-cloud/campaign/reporting/get-started-reporting){target="_blank"}可用於報告電子郵件傳送、包含電子郵件傳送的行銷活動以及異動訊息。 也可使用依設定檔維度的人口統計分析。
>
